...
exports.getUserFeedback = exports.getFeedbackStats = exports.updateFeedbackStatus = exports.voteFeedback = exports.getFeedbackById = exports.getFeedback = exports.createFeedback = void 0;
...
const createFeedback = async (req, res) => {
    try {
        const { type, category, priority = 'medium', rating, title, description, steps, expectedBehavior, actualBehavior, email, browserInfo, context, tags = [] } = req.body;
        // Validation
        ...
        if (type === 'bug' && !priority) {
            ...
            const hasCriticalKeyword = criticalKeywords.some(keyword => description.toLowerCase().includes(keyword) || title.toLowerCase().includes(keyword));
...
const getFeedback = async (req, res) => {
    try {
        const { page = 1, limit = 10, type, category, status, priority, userId, search, sortBy = 'createdAt', sortOrder = 'desc', tags } = req.query;
        // Build filter query
        const filter = {};
        if (type)
            filter.type = type;
        ...
        // Execute query
        const feedback = await Feedback_1.default.find(filter)
            .sort(sortOptions)
            .skip(skip)
            .limit(limitNum)
            .populate('userId', 'name email')
            .populate('resolvedBy', 'name email');
        const total = await Feedback_1.default.countDocuments(filter);
        res.json({
            success: true,
            data: {
                feedback,
...
const updateFeedbackStatus = async (req, res) => {
    try {
        ...
        const feedback = await Feedback_1.default.findByIdAndUpdate(id, updateData, { new: true }).populate('userId', 'name email')
            .populate('resolvedBy', 'name email');
        if (!feedback) {
            return res.status(404).json({
                success: false,
...
const getFeedbackStats = async (req, res) => {
    try {
        const stats = await Feedback_1.default.getFeedbackStats();
        const topIssues = await Feedback_1.default.getTopIssues(5);
        // Recent feedback trends (last 30 days)
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        const recentTrends = await Feedback_1.default.aggregate([
            {
                $match: {
