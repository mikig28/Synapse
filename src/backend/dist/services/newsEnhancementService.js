...
exports.enhanceNewsItemWithImage = enhanceNewsItemWithImage;
exports.enhanceNewsItemsBatch = enhanceNewsItemsBatch;
exports.enhanceRecentNewsItems = enhanceRecentNewsItems;
exports.getImageEnhancementStats = getImageEnhancementStats;
 ...
 */
async function enhanceNewsItemWithImage(newsItem, options = {}) {
    try {
 ...
 */
async function enhanceNewsItemsBatch(newsItems, options = {}) {
    const { batchSize = 5, skipExisting = true, maxRetries = 2 } = options;
    let enhanced = 0;
    let failed = 0;
    let skipped = 0;
 ...
 */
async function enhanceRecentNewsItems(userId, hoursBack = 24, options = {}) {
    const cutoffDate = new Date();
    cutoffDate.setHours(cutoffDate.getHours() - hoursBack);
    const newsItems = await NewsItem_1.default.find({
        userId,
 ...
 */
function generateImagePrompt(newsItem) {
 ...
 */
async function getImageEnhancementStats(userId) {
    const [total, withImages, withUnsplash, withReplicate] = await Promise.all([
        NewsItem_1.default.countDocuments({ userId }),
        NewsItem_1.default.countDocuments({ userId, 'generatedImage.url': { $exists: true } }),
        NewsItem_1.default.countDocuments({ userId, 'generatedImage.source': 'unsplash' }),
        NewsItem_1.default.countDocuments({ userId, 'generatedImage.source': 'replicate' })
    ]);
    return {
        total,
        withImages,
