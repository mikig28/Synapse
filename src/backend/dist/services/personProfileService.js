...
class PersonProfileService {
     ...
     */
    async createPersonProfile(userId, profileData) {
        try {
            // First create the person profile in database
            const personProfile = new PersonProfile_1.default({
            ...
            try {
                const faceRecognitionResponse = await this.registerWithFaceRecognition(savedProfile._id.toString(), profileData.name, profileData.trainingImages);
     ...
     */
    async getPersonProfilesByUser(userId) {
        try {
            return await PersonProfile_1.default.find({
                userId: new mongoose_1.Types.ObjectId(userId),
                isActive: true
            }).sort({ name: 1 });
        }
     ...
     */
    async getPersonProfileById(profileId, userId) {
        try {
            return await PersonProfile_1.default.findOne({
     ...
     */
    async updatePersonProfile(profileId, userId, updates) {
        try {
            const profile = await PersonProfile_1.default.findOne({
                _id: new mongoose_1.Types.ObjectId(profileId),
                userId: new mongoose_1.Types.ObjectId(userId)
            });
            if (!profile) {
            ...
            if (updates.trainingImages && updates.trainingImages.length > 0) {
                ...
                try {
                    await this.registerWithFaceRecognition(profileId, profile.name, updates.trainingImages);
                }
                catch (faceError) {
     ...
     */
    async deletePersonProfile(profileId, userId) {
        try {
            const profile = await PersonProfile_1.default.findOne({
                _id: new mongoose_1.Types.ObjectId(profileId),
                userId: new mongoose_1.Types.ObjectId(userId)
     ...
     */
    async addTrainingImages(profileId, userId, newImages) {
        try {
            const profile = await PersonProfile_1.default.findOne({
                _id: new mongoose_1.Types.ObjectId(profileId),
                userId: new mongoose_1.Types.ObjectId(userId),
            ...
            try {
                await this.registerWithFaceRecognition(profileId, profile.name, allImages);
            }
            catch (faceError) {
     ...
     */
    async registerWithFaceRecognition(personId, personName, trainingImages) {
        try {
            const response = await axios_1.default.post(`${this.faceRecognitionUrl}/api/face/register`, {
                personId,
                personName,
                trainingImages
            }, {
     ...
     */
    async deleteFaceRecognitionData(personId) {
        try {
            await axios_1.default.delete(`${this.faceRecognitionUrl}/api/face/persons/${personId}`, { timeout: 10000 });
        }
        catch (error) {
    ...
    async testFaceRecognitionService() {
        try {
            const response = await axios_1.default.get(`${this.faceRecognitionUrl}/health`, { timeout: 5000 });
...
exports.default = PersonProfileService;
