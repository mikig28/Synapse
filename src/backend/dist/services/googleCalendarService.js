...
class GoogleCalendarService {
     ...
     */
    async processPendingDeletions(userId) {
        try {
            ...
            const ourGoogleEvents = await CalendarEvent_1.default.find({
                userId,
            ...
            const googleEvents = response.data.items || [];
            const googleEventIds = new Set(googleEvents.map(e => e.id).filter((id) => Boolean(id)));
            const ourGoogleEventIds = new Set(ourGoogleEvents.map(e => e.googleEventId).filter((id) => Boolean(id)));
            ...
            const eventsToDeleteFromGoogle = [];
