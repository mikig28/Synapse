...
class VectorDatabaseService {
    constructor() {
        ...
        const primaryProvider = process.env.EMBEDDING_PROVIDER || 'voyage';
        const fallbackProviders = (process.env.EMBEDDING_FALLBACK_PROVIDERS || 'gemini,openai').split(',');
        this.config = {
    ...
    getDimensionalityForProvider(provider) {
        switch (provider) {
            case 'voyage':
        ...
        try {
            ...
            if (!this.config.useProduction) {
                try {
                    ...
                    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('ChromaDB heartbeat timeout')), 10000));
    ...
    async initializePineconeIndex() {
        ...
        try {
            ...
            const existingIndex = existingIndexes.indexes?.find(index => index.name === this.config.pineconeIndexName);
            if (!existingIndex) {
     ...
     */
    async generateEmbeddingWithProvider(text, provider) {
     ...
     */
    async generateVoyageEmbedding(text, retryCount = 0) {
        ...
        try {
            ...
            const response = await axios_1.default.post('https://api.voyageai.com/v1/embeddings', {
                input: [text],
    ...
    async generateGeminiEmbedding(text) {
        ...
        const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/text-embedding-004:embedContent?key=${this.config.geminiApiKey}`, {
            content: {
                parts: [{ text: text }]
            },
     ...
     */
    truncateTextForProvider(text, provider) {
        let maxTokens;
        switch (provider) {
            case 'voyage':
    ...
    async healthCheck() {
        ...
        const providers = ['voyage', 'gemini', 'openai'];
        for (const provider of providers) {
