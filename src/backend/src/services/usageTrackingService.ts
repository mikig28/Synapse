...
export class UsageTrackingService {
   ...
   */
  async trackUsage(data: UsageEventData): Promise<void> {
    try {
   ...
   */
  private incrementUsageCounters(usage: IUsage, data: UsageEventData): void {
    const { feature, action, metadata } = data;
    
    switch (feature) {
      case 'search':
        usage.features.searches.count += 1;
        if (metadata?.unique) usage.features.searches.uniqueQueries += 1;
        if (metadata?.resultsCount) usage.features.searches.totalResultsReturned += metadata.resultsCount;
        if (metadata?.isVector) usage.features.advanced.vectorSearchQueries += 1;
        break;
        
      case 'agent':
        usage.features.agents.executionsCount += 1;
        if (data.computeTime) usage.features.agents.totalExecutionTime += data.computeTime;
        if (metadata?.agentId && !metadata.counted) usage.features.agents.uniqueAgentsUsed += 1;
        if (action === 'schedule') usage.features.agents.scheduledAgentsCount += 1;
        break;
        
      case 'document':
        if (action === 'upload') usage.features.data.documentsUploaded += 1;
        if (action === 'analyze') usage.features.data.documentsAnalyzed += 1;
        if (data.storageUsed) usage.features.data.totalStorageUsed += data.storageUsed;
        break;
      ...
      case 'note':
        if (action === 'create') usage.features.content.notesCreated += 1;
        break;
        
      case 'idea':
        if (action === 'create') usage.features.content.ideasCreated += 1;
        break;
        
      case 'task':
        if (action === 'create') usage.features.content.tasksCreated += 1;
        break;
        
      case 'meeting':
        if (action === 'create') usage.features.content.meetingsCreated += 1;
        break;
        
      case 'bookmark':
        if (action === 'add') usage.features.content.bookmarksAdded += 1;
        break;
        
      case 'ai':
        if (action === 'summarize') usage.features.advanced.aiSummariesGenerated += 1;
        break;
   ...
   */
  private async getPreviousPeriodUsage(
    userId: mongoose.Types.ObjectId, 
    periodType: 'daily' | 'weekly' | 'monthly',
    currentStart: Date
  ): Promise<IUsage | null> {
   ...
   */
  private getPeriodBounds(date: Date, periodType: 'daily' | 'weekly' | 'monthly') {
    switch (periodType) {
      case 'daily':
  ...
  async getUsageAnalytics(
    ...
    const revenueProjection = (
      (tierDist.starter || 0) * 29 + // $29/month for starter
   ...
   */
  async trackSearch(userId: string, responseTime: number, resultsCount: number, isVector: boolean = false): Promise<void> {
  ...
  async trackAgentExecution(userId: string, agentId: string, computeTime: number): Promise<void> {
    await this.trackUsage({
      userId,
      feature: 'agent',
      action: 'execute',
  ...
  async trackContentCreation(userId: string, contentType: 'note' | 'idea' | 'task' | 'meeting' | 'bookmark'): Promise<void> {
    await this.trackUsage({
      userId,
      feature: contentType,
      action: 'create'
  ...
  async trackIntegrationMessage(userId: string, platform: 'whatsapp' | 'telegram'): Promise<void> {
    await this.trackUsage({
      userId,
      feature: platform,
      action: 'message'
  ...
  async trackExport(userId: string, format: string, itemCount: number): Promise<void> {
    await this.trackUsage({
      userId,
      feature: 'export',
      action: 'create',
