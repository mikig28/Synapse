...
class WhatsAppBaileysService extends EventEmitter {
  ...
  private async processChatUpdates(chats: any[]): Promise<void> {
    ...
    try {
      ...
      const groupId = isGroup ? msg.chatId : undefined;
      const groupName = isGroup ? (msg.groupName || undefined) : undefined;

      const ts = typeof msg.timestamp === 'number' && msg.timestamp < 2e10
        ? new Date(msg.timestamp * 1000)
        : new Date(msg.timestamp);

      const fromJid = msg.from || '';
      const fromPhone = fromJid.includes('@') ? fromJid.split('@')[0] : fromJid;
      ...
      if (!contact) {
        contact = await WhatsAppContactModel.create({
          ...
          name: msg.contactName || fromPhone,
          lastSeen: ts,
          isOnline: false,
          unreadCount: 0,
