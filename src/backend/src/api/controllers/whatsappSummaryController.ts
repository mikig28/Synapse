    ...
    res.json({
      ...
      data: {
        ...
        messageTypes: {
          ...
          other: Object.values(typeStats).reduce((sum: number, count: any) => sum + (Number(count) || 0), 0) - 
                 (Number(typeStats.text) || 0) - (Number(typeStats.image) || 0) - (Number(typeStats.video) || 0) - 
                 (Number(typeStats.audio) || 0) - (Number(typeStats.document) || 0)
        },
