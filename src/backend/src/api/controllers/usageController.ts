...
export const getUserUsage = async (req: AuthenticatedRequest, res: express.Response) => {
  try {
    ...
    const usage = await usageTrackingService.getUserUsage(userId, periodType);
...
export const checkUsageLimit = async (req: AuthenticatedRequest, res: express.Response) => {
  try {
    ...
    const result = await usageTrackingService.canUserPerformAction(userId, action, feature);
...
export const getUsageAnalytics = async (req: AuthenticatedRequest, res: express.Response) => {
  try {
    ...
    const analytics = await usageTrackingService.getUsageAnalytics(start, end);
...
export const simulateTierUpgrade = async (req: AuthenticatedRequest, res: express.Response) => {
  try {
    ...
    const usage = await usageTrackingService.getUserUsage(userId, 'monthly');
    ...
    if (usage) {
      ...
      usage.billing.credits.remaining = creditsMap[tier as keyof typeof creditsMap];
